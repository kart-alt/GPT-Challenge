...
Add TensorFlow Lite Model (Optional for full functionality)

Place distress_audio_model.tflite in app/src/main/assets/
See Model Training Guide for details
Build & Run
...
📱 Usage
First Launch
Grant permissions (microphone, location, SMS)
Add trusted contacts (at least 1)
...
Local alert: Loud sound + vibration
Cancel window: 10 seconds to cancel
If not cancelled: SMS sent to contacts with location
Modes
...
MVP (Week 1) ✅
 ...
 Background service
 Alert system with cancel window
 SMS notification
 Basic UI
Future Enhancements
...
Privacy-first (no cloud, no recording)
Multi-modal detection (audio + motion + context)
False alarm prevention (context-aware)
Open-source and auditable
📚 Citations
Datasets Used
